...
namespace Drupal\entity_browser_generic_embed\Hook;
...
/**
 * Hook implementations for the Entity Browser Generic Embed module.
 */
class EntityBrowserGenericEmbedHooks {

  use StringTranslationTrait;
  use DependencySerializationTrait;

  /**
   * Constructs an EntityBrowserGenericEmbedHooks object.
   *
   ...
   * @param object $mediaHelper
   *   The entity browser generic embed media helper.
   * @param object $fileValidator
   *   The file validator.
   */
  public function __construct(
    ...
    #[Autowire(service: 'entity_browser_generic_embed.media_helper')]
    protected $mediaHelper,
    #[Autowire(service: 'file.validator')]
    protected $fileValidator,
  ) {}

  /**
   * Implements hook_form_media_type_form_alter().
   */
  #[Hook('form_media_type_form_alter')]
  public function formMediaTypeFormAlter(array &$form, FormStateInterface $form_state) {
  ...
  /**
   * Implements hook_view_insert().
   */
  #[Hook('view_insert')]
  public function viewInsert(ViewEntityInterface $view) {
  ...
  public function elementInfoAlter(array &$info) {
    $info['entity_browser']['#after_build'][] = [$this, 'injectEntityBrowserCount'];
  }
      ...
      foreach (MediaType::loadMultiple() as $id => $media_type) {
        ...
        if (isset($plugin_definition['preview'])) {
          $extra_fields['media'][$id]['form']['preview'] = [
            ...
            'description' => $this->t('A live preview of the @media_type.', [
  ...
  public function inlineEntityFormEntityFormAlter(array &$entity_form) {
   ...
   */
  public function validateUpload(FileInterface $file, array $bundles = []) {
    try {
      $entity = $this->mediaHelper->createFromInput($file, $bundles);
    }
   ...
   */
  public function injectEntityBrowserCount(array $element) {
  ...
  /**
   * Implements hook_form_entity_embed_dialog_alter().
   */
  #[Hook('form_entity_embed_dialog_alter')]
  public function formEntityEmbedDialogAlter(array &$form, FormStateInterface $form_state) {
  ...
  public function preprocessViewsViewGrid(array &$variables) {
  ...
  public function entityFormDisplayPresave(EntityFormDisplayInterface $display) {
